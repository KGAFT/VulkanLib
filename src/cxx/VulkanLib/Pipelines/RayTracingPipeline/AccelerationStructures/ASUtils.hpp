...
namespace vkLibRt {
    class ASUtils {
    public:
        static void cmdCreateBlas(std::shared_ptr<LogicalDevice> device, Instance &instance, vk::CommandBuffer cmdBuf,
                                  std::vector<uint32_t> indices,
                                  std::vector<BuildAccelerationStructure> &buildAs,
                                  VkDeviceAddress scratchAddress,
                                  vk::QueryPool queryPool) {
            if (queryPool)  // For querying the compaction size
                device->getDevice().resetQueryPool(queryPool, 0, static_cast<uint32_t>(indices.size()));
            uint32_t queryCnt{0};
            ...
            for (const auto &idx: indices) {
                ...
                buildAs[idx].as = createAcceleration(device, createInfo, instance.getDynamicLoader());
                ...
                if (queryPool) {
                    ...
                    cmdBuf.writeAccelerationStructuresPropertiesKHR(1,
                                                                    &buildAs[idx].buildInfo.dstAccelerationStructure,
                                                                    vk::QueryType::eAccelerationStructureCompactedSizeKHR,
                                                                    queryPool, queryCnt++, instance.getDynamicLoader());

                }
            }
        }

        static vk::DeviceAddress getASAddress(std::shared_ptr<LogicalDevice> device, vk::AccelerationStructureKHR accel, vk::DispatchLoaderDynamic &loaderDynamic){
            vk::AccelerationStructureDeviceAddressInfoKHR addressInfo{};
            addressInfo.accelerationStructure = accel;
            return device->getDevice().getAccelerationStructureAddressKHR(addressInfo, loaderDynamic);
        }
        ...
        static void cmdCreateTlas(std::shared_ptr<LogicalDevice> device, Instance &instance, vk::CommandBuffer cmdBuf, Buffer* scratchBuffer,
            ...
            if(update == false)
            {
                ...
                tlas = createAcceleration(device, createInfo, instance.getDynamicLoader());
        ...
        static void
        objectToVkGeometryKHR(std::shared_ptr<VertexBuffer> vBuffer, std::shared_ptr<IndexBuffer> indexBuffer,
                              BlasInput *pOutObjectGeomtry, vk::DispatchLoaderDynamic &loaderDynamic) {
            vk::DeviceAddress vertexAddress = vBuffer->getBufferAddress(loaderDynamic);
            vk::DeviceAddress indexAddress = indexBuffer->getBufferAddress(loaderDynamic);

            uint32_t maxPrimitiveCount = indexBuffer->getIndexCount() / 3;
            pOutObjectGeomtry->asGeometry.push_back({});
            pOutObjectGeomtry->asBuildOffsetInfo.push_back({});
            pOutObjectGeomtry->asGeometry[pOutObjectGeomtry->asGeometry.size() -
                                          1].geometry.triangles.vertexFormat = vBuffer->getFormat();  // vec3 vertex position data.
            pOutObjectGeomtry->asGeometry[pOutObjectGeomtry->asGeometry.size() -
                                          1].geometry.triangles.vertexData.deviceAddress = vertexAddress;
            pOutObjectGeomtry->asGeometry[pOutObjectGeomtry->asGeometry.size() -
                                          1].geometry.triangles.vertexStride = vBuffer->getStepSize();
            // Describe index data (32-bit unsigned int)
            pOutObjectGeomtry->asGeometry[pOutObjectGeomtry->asGeometry.size() -
                                          1].geometry.triangles.indexType = indexBuffer->getIndexType();
            pOutObjectGeomtry->asGeometry[pOutObjectGeomtry->asGeometry.size() -
                                          1].geometry.triangles.indexData.deviceAddress = indexAddress;
            // Indicate identity transform by setting transformData to null device pointer.
            //triangles.transformData = {};
            pOutObjectGeomtry->asGeometry[pOutObjectGeomtry->asGeometry.size() - 1].geometry.triangles.maxVertex =
                    vBuffer->getVerticesAmount() - 1;

            // Identify the above data as containing opaque triangles.
            pOutObjectGeomtry->asGeometry[pOutObjectGeomtry->asGeometry.size() -
                                          1].geometryType = vk::GeometryTypeKHR::eTriangles;
            pOutObjectGeomtry->asGeometry[pOutObjectGeomtry->asGeometry.size() -
                                          1].flags = vk::GeometryFlagBitsKHR::eOpaque;

            pOutObjectGeomtry->asBuildOffsetInfo[pOutObjectGeomtry->asBuildOffsetInfo.size() - 1].firstVertex = 0;
            pOutObjectGeomtry->asBuildOffsetInfo[pOutObjectGeomtry->asBuildOffsetInfo.size() -
                                                 1].primitiveCount = maxPrimitiveCount;
            pOutObjectGeomtry->asBuildOffsetInfo[pOutObjectGeomtry->asBuildOffsetInfo.size() - 1].primitiveOffset = 0;
            pOutObjectGeomtry->asBuildOffsetInfo[pOutObjectGeomtry->asBuildOffsetInfo.size() - 1].transformOffset = 0;
        }

        static void cmdCompactBlas(std::shared_ptr<LogicalDevice> device, vk::CommandBuffer cmdBuf,
                                   std::vector<uint32_t> indices,
                                   std::vector<BuildAccelerationStructure> &buildAs,
                                   VkQueryPool queryPool, vk::DispatchLoaderDynamic &loader) {
            uint32_t queryCtn{0};

            // Get the compacted size result back
            std::vector<VkDeviceSize> compactSizes(static_cast<uint32_t>(indices.size()));

            vkGetQueryPoolResults(device->getDevice(), queryPool, 0, (uint32_t) compactSizes.size(),
        ...
        static AccelKHR
        createAcceleration(std::shared_ptr<LogicalDevice> device, vk::AccelerationStructureCreateInfoKHR &accel_,
