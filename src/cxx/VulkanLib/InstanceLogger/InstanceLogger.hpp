...
public:
    InstanceLogger(vk::Instance &instance, vk::DispatchLoaderDynamic &dynamicLoader) : instance(instance), loaderDynamic(dynamicLoader){
...
private:
    VkBool32 debugCall(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
                           VkDebugUtilsMessageTypeFlagsEXT messageType,
                           const VkDebugUtilsMessengerCallbackDataEXT *pCallbackData) {
        ...
        for (const auto &item: loggerCallbacks){
            switch (item->getCallBackMode()) {
                case RAW_VULKAN_DEFS:
                    item->messageRaw(messageSeverity, messageType, pCallbackData, nullptr);
                    break;
        ...
        if(loggerCallbacks.empty()){
            ...
            defaultCallback.translatedMessage(severityTranslated, typeTranslated, message);
        }
        return VK_FALSE;
    }
    const char* translateSeverity(VkDebugUtilsMessageSeverityFlagBitsEXT severity)
    {
        const char *res;
        switch (severity)
        {
